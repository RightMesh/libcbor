...
public class CborEncoderTest {
    ...
    public void encodeAppendixA_Byte_Text_Strings_Definite() {
        ...
        byte[] a4 = {0x01, 0x02};
        byte[] a5 = {0x03, 0x04, 0x05};
        Flowable<ByteBuffer> source = Flowable.just(ByteBuffer.wrap(a4), ByteBuffer.wrap(a5))
                .map(b -> {
                    b.position(0);
                    return b;
                });
        enc.cbor_encode_byte_string(5, source);
        assertEquals("0x450102030405", getEncodedString());

        enc.cbor_encode_byte_string(source, true);
        assertEquals("0x450102030405", getEncodedString());
